...
df = pd.read_csv(path_vocabulary, header=0, encoding="utf-8", sep=";", index_col=False)
...
for topic_ind, topic in enumerate(nmf.components_):
    ...
    for i in range(len(feature_names)):
        ...
        if weight > 0:
            d[word] = weight
    wc = WordCloud(width=1000, height=500, margin=2, prefer_horizontal=0.9, background_color='white', colormap="viridis")
    wc = wc.fit_words(d)
    plt.figure()
